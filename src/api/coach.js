...
export const getCoachDetailsByCourse = (courseId) => {
  ...
  return axiosInstance
    .get(`/parent/coach/by-course/${id}`, {
      _noLogoutOn401: true,
      _noBlockOn403: true,
    })
    .then((response) => {
      console.log(
        `[API] Coach details API response for course ${id}:`,
        response.data
      );
    ...
    .catch((error) => {
      ...
      if (isNotFound) {
        console.log(
          `[API] Coach not found via parent API for course ${id}, trying course details fallback...`
        );
        return axiosInstance
          .get(`/courses/${id}`)
          .then((courseResponse) => {
            console.log(
              `[API] Course details fallback response for course ${id}:`,
              courseResponse.data
            );
            const course = courseResponse.data?.data || courseResponse.data;
            if (course && course.coach) {
              console.log(
                `[API] Found coach data in course details for course ${id}:`,
                course.coach
              );
              return {
                data: {
                  ...
                  data: {
                    ...
                    firstName:
                      ...
                      course.coach.name?.split(" ")[0] ||
                      "Coach",
                    lastName:
                      course.coach.lastName ||
                      course.coach.name?.split(" ")[1] ||
                      "",
                    ...
                    domain:
                      course.coach.domain || course.coach.specialization || "",
              ...
              console.error(
                `[API] No coach data found in course details for course ${id}`
              );
              throw new Error(
                "Coach information not available for this course"
              );
            }
          })
          .catch((fallbackError) => {
            console.error(
              `[API] Fallback course details also failed for course ${id}:`,
              fallbackError.response?.data || fallbackError.message
            );
            throw new Error(
              "Unable to retrieve coach information for this course"
            );
          });
      }
      ...
      console.log(
        `[API] Not a 404 error, throwing original error for course ${id}`
      );
      throw error;
    });
};
  ...
  window.testAPIs = async (courseId) => {
    ...
    try {
      ...
      const response1 = await fetch(
        `http://localhost:5000/api/v1/parent/coach/by-course/${courseId}`
      );
  ...
  window.testCoachAPISimple = async (courseId) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/parent/coach/by-course/${courseId}`
      );
